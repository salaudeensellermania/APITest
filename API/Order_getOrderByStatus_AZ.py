...
#Purpose: To do a regression check for GetOrderByDate Call (to see if the call is success, all the data are properly fetched from DB for Amazon (cso TABLE), and also the tags and data are validated)
...
import csv
import datetime
from apipackages import xml_order_parse
...
#DB connect
		try:
			...
			cursor.execute (row['query_delete_cso.amazon'])
			cursor.execute (row['query_insert_cso.amazon'])			
			print("\n")
			outfile.write("\n")
...
#OOPS		
		obj1=xml_order_parse.getorderbystatus()
#call1
...
#Get Status code and print the results
	...
	#			print i
				...
				if i==0:
					...
					outfile.write("\n")
					print ("-------------------------------------------")
					outfile.write("ORDER API - GetOrderByStatus --> PASSED")
					outfile.write("\n")
					print ("ORDER API - GetOrderByStatus --> PASSED")
					outfile.write("-------------------------------------------")
					print ("-------------------------------------------")
					outfile.write("\n")
					print ("\n")
					...
					outfile.write("\n")
					print ("---------------------------------------------------------------------------")
					outfile.write("ORDER API - GetOrderByStatus --> PASSED WITH ERROR / DATA MISMATCH")
					outfile.write("\n")
					print ("ORDER API - GetOrderByStatus --> PASSED WITH ERROR / DATA MISMATCH")
					outfile.write("---------------------------------------------------------------------------")
					print ("---------------------------------------------------------------------------")
					outfile.write("\n")
					print ("\n")
	...
	#call3
				...
				outfile.write("\n")
				print ("-------------------------------------------")
				outfile.write("ORDER API - GetOrderByStatus --> FAILED")
				outfile.write("\n")
				print ("ORDER API - GetOrderByStatus --> FAILED")
				outfile.write("-------------------------------------------")
				print ("-------------------------------------------")
				outfile.write("\n")
				print ("\n")
